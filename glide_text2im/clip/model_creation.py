...
class CLIPModel:
    ...
    def text_embeddings(self, prompts: List[str]) -> torch.Tensor:
        ...
        # @domll: text_encoder returns a single token that is a rappresentation of our input prompt
    ...
    def cond_fn(self, prompts: List[str], grad_scale: float) -> Callable[..., torch.Tensor]:
        with torch.no_grad():
            # @domll: we are gonna form the embeddings for the prompts
            z_t = self.text_embeddings(prompts)

        # @domll: then we will return this conditioning function
        def cond_fn(x, t, grad_scale=grad_scale, **kwargs):
            ...
            # @domll: we find the gradient of the loss multiplied with the gradient scale (3.0 by default in notebook[s]);
            # so this gradient is gonna move our image towards the part of the space where that image resembles 
            # the prompt much more closely compared to the unconditional case
