...
class Text2ImUNet(UNetModel):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        else:   # @domll: we call the constructor of the super class (simple UNetModel)
            super().__init__(*args, **kwargs, encoder_channels=xf_width)
        if self.xf_width:
            self.transformer = Transformer(# @domll: here is the difference between previous paper, the Transformer
                text_ctx,
                xf_width,
                xf_layers,
                xf_heads,
            ...
            # @domll: going to embed tokens into a certain vector space
            self.token_embedding = nn.Embedding(self.tokenizer.n_vocab, xf_width)
            # @domll: we have positional_embedding, we don't need more than 128 because
            # that's going to be the max context that this transformer is going to use
            self.positional_embedding = nn.Parameter(th.empty(text_ctx, xf_width, dtype=th.float32))
            # @domll: transformer projection linear layer
            self.transformer_proj = nn.Linear(xf_width, self.model_channels * 4)

            # @domll: padding embedding
            if self.xf_padding:
    ...
    def get_text_emb(self, tokens, mask):
        ...
        # @domll: we are returning not just the last token, but all of the tokens of
        # the embeddings vectors from the previous layer
        # 512 tokens
    ...
    def forward(self, x, timesteps, tokens=None, mask=None):
        hs = []
        # @domll: how text information is used to condition the model
        # embed the time_steps
        emb = self.time_embed(timestep_embedding(timesteps, self.model_channels))
        if self.xf_width:
            text_outputs = self.get_text_emb(tokens, mask)  # @domll: pass tokens and mask to the transformer part
            xf_proj, xf_out = text_outputs["xf_proj"], text_outputs["xf_out"]
            # @domll: we are doing this in the same way as if we were added just a simple class information
            emb = emb + xf_proj.to(emb)
        else:
            xf_out = None
        h = x.type(self.dtype)
        # @domll: we are passing xf_out throughout various modules of our unet model
        # THIS IS HOW we are conditioning using the temporal information + the textual information, 
        # the real difference between glide and previous implementation
        # emb is temporal, xf_out is textual
        # emb is used by unet.py->ResBlock, xf_out is used by unet.py->AttentionBlock
